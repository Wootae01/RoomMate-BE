...
public class AuthController {
    ...
    public ResponseEntity<JWTTokenDTO> reissue(HttpServletRequest request) {
        ...
        // 기존 refresh 토큰 삭제, 새로운 토큰 생성 후 저장
        JWTTokenDTO newToken = refreshService.createNewToken(refresh);


        refreshService.deleteByRefresh(refresh);

        refreshService.save(jwtUtil.getUsername(refresh), newToken.getRefreshToken(),
    ...
    public ResponseEntity<LoginResponseDTO> login(@RequestBody LoginRequestDTO dto) {
        ...
        if (optional.isEmpty()) {
            ...
            refreshService.save(username, response.getRefreshToken(),
                    JWTConstants.REFRESH_TOKEN_EXPIRATION);
