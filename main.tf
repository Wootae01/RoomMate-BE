...
output "ssh_command" {
  value = "ssh -i roommate-key.pem ubuntu@${aws_eip.app.public_ip}"
}
...
# RDS 인스턴스
resource "aws_db_instance" "roommate" {
  identifier        = "roommate-db"
  engine            = "mysql"
  engine_version    = "8.0"
...
# RDS 엔드포인트 출력
output "rds_endpoint" {
  value = aws_db_instance.roommate.endpoint
}

# CloudWatch 대시보드 생성
resource "aws_cloudwatch_dashboard" "main" {
  dashboard_name = "roommate-dashboard"
    ...
    widgets = [
      ...
      },
      {
        ...
        properties = {
          ...
          stacked = false
          metrics = [
            ["AWS/RDS", "CPUUtilization", "DBInstanceIdentifier", aws_db_instance.roommate.identifier]
          ]
          region = "ap-northeast-2"
          title  = "RDS CPU 사용률 (%)"
          period = 300
      ...
      },
      {
        ...
        properties = {
          ...
          stacked = false
          metrics = [
            ["AWS/RDS", "DatabaseConnections", "DBInstanceIdentifier", aws_db_instance.roommate.identifier],
            ["AWS/RDS", "FreeableMemory", "DBInstanceIdentifier", aws_db_instance.roommate.identifier],
            ["AWS/RDS", "FreeStorageSpace", "DBInstanceIdentifier", aws_db_instance.roommate.identifier]
          ]
          region = "ap-northeast-2"
          title  = "RDS 상태"
          period = 300
